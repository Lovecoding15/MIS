rem ' ****************************************************
rem ' * SPROC to drive customer invoices for BSG in German
rem ' ****************************************************
rem ' program name: custInvoicesBSGde.prc
...
call "ec_open::ARM10B"; rem ' tax code table
...
build_repeating_data:
	...
	if ust_id$ <> "" then
		...
		footer2$ = "Your VAT ID: " + ust_id$
		footer3$ = "Our VAT ID: DE 194403119"
	else
		footer1$ = ""
		footer2$ = ""
		footer3$ = ""
    ...
    if fromhistory then
        tax_code$ = art03.tax_code$ 
    else
        tax_code$ = are03.tax_code$
    fi
    if len(cvs(tax_code$,3)) = 2 then
        read record(arm10b, key = firm_id$ + "B" + tax_code$)arm10b$
        tax_rate$ = arm10b.tax_rate$ + "%"
    else
        tax_rate$ = ""
    fi
...
dataTemplate$ = dataTemplate$ + "footer1:C(30),footer2:C(30),footer3:C(30)"
	...
	data!.setFieldValue("TAX_RATE", tax_rate$)
	...
	if fromhistory then
		...
		if found then
			...
			if are73.type_of_sale$ = "SM1" or are73.exp_type$ = "RN1" or are73.exp_type$ = "KIT" or are73.exp_type$ = "NFR" then	
				...
				description$ = description$ + " Laufzeit bis " + annual_dt$
			fi
		fi
	fi
...
sn_enduser_line_setup:
	...
	if len(deactlist$) > 0 then
		...
		data! = rs!.getEmptyRecordData()
		gosub setup_repeating_data
		description$ = "Deaktivierte Seriennummer:"  
		data!.setFieldValue("DESCRIPTION", description$)
		rs!.insert(data!)
		line = line + 1
	...
	if len(modlist$) > 0 then
		...
		if cvs(po_number$,3) = "SAM Renew" or cvs(po_number$,3) = "DVK Renew" or cvs(po_number$,3) = "RNT Renew" then
			description$ = "Seriennummer:          End User:"
		else
